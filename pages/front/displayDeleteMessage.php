...
$queryPrepared = $connect->prepare('SELECT m.idMessagerie,m.titre,m.texte,m.dateEnvoie FROM messagerie m INNER JOIN personne p1 ON p1.idPersonne = m.idSource WHERE m.idDestinataire = ? AND p1.statut = 2 AND m.statutDestinataire = 1 ORDER BY m.dateEnvoie desc;');
 $queryPrepared->execute([$_SESSION['user']['idPersonne']]);
 $array = $queryPrepared->fetchAll();


$queryPrepared = $connect->prepare('SELECT m.idMessagerie,m.titre,m.texte,m.dateEnvoie FROM messagerie m INNER JOIN personne p ON p.idPersonne = m.idSource WHERE m.idSource = ? AND m.statutDestinataire = 1 ORDER BY m.dateEnvoie desc;');
$queryPrepared->execute([$_SESSION['user']['idPersonne']]);
